...
from adafruit_register import i2c_struct, i2c_bits
...
_CMD_START_MEASUREMENT = const(0x0021)
_CMD_STOP_MEASUREMENT = const(0x0104)
_CMD_READ_DATA_READY = const(0x0202)
_CMD_RESET = const(0xD304)
_CMD_READ_SERIAL_NUMBER = const(0xD033)
_CMD_READ_PRODUCT_NAME = const(0xD014)
_CMD_READ_DEVICE_STATUS = const(0xD206)
_CMD_CLEAR_DEVICE_STATUS = const(0xD210)
_CMD_START_FAN_CLEANING = const(0x5607)
_CMD_GET_VOC_STATE = const(0x6181)
_CMD_SET_VOC_STATE = const(0x6181)

# Temperature configuration commands
_CMD_SET_TEMP_OFFSET = const(0x60B2)
_CMD_SET_TEMP_ACCELERATION = const(0x6100)

# SEN66-specific commands
_CMD_SEN66_READ_MEASUREMENT = const(0x0300)  # Read all measurements
_CMD_SEN66_READ_RAW_VALUES = const(0x0405)  # Read raw values

# Command execution times (in seconds)
_EXEC_TIME_START_MEASUREMENT = const(0.050)  # 50ms
_EXEC_TIME_STOP_MEASUREMENT = const(1.000)   # 1000ms
_EXEC_TIME_READ_DATA_READY = const(0.020)    # 20ms
_EXEC_TIME_READ_MEASUREMENT = const(0.020)    # 20ms
_EXEC_TIME_STANDARD = const(0.020)            # 20ms for most commands
_EXEC_TIME_MINIMAL = const(0.001)             # 1ms minimum wait
_EXEC_TIME_RESET = const(1.200)               # 1200ms for reset
_EXEC_TIME_SHT_HEATER = const(1.300)          # 1300ms for SHT heater
_EXEC_TIME_CO2_RECALIBRATION = const(0.500)   # 500ms for CO2 recalibration
...
_UNKNOWN_VALUE_UINT16 = const(0xFFFF)  # Unknown value for unsigned 16-bit
_UNKNOWN_VALUE_INT16 = const(0x7FFF)   # Unknown value for signed 16-bit
...
class DeviceStatus:
    ...
    def has_errors(self):
        ...
        error_mask = ((1 << _STATUS_CO2_1_ERROR) | 
                      (1 << _STATUS_PM_ERROR) | 
...
class SEN6x:
    ...
    def __init__(self, i2c, address=SEN6X_I2C_ADDRESS):
    ...
    def _write_command(self, command, data=None, execution_time=_EXEC_TIME_STANDARD):
    ...
    def _read_data(self, num_words, execution_time=_EXEC_TIME_STANDARD):
        """Read data from sensor after a read command
    ...
    def reset(self):
        ...
        """
        self._write_command(_CMD_RESET, execution_time=_EXEC_TIME_RESET)
        self._measurement_started = False
        # Clear cached values
        self._serial_number = None
        self._product_name = None
    ...
    def start_measurement(self):
        ...
        if self._measurement_started:
            return
        self._write_command(_CMD_START_MEASUREMENT, execution_time=_EXEC_TIME_START_MEASUREMENT)
    ...
    def stop_measurement(self):
        ...
        if not self._measurement_started:
            return
        self._write_command(_CMD_STOP_MEASUREMENT, execution_time=_EXEC_TIME_STOP_MEASUREMENT)
        self._measurement_started = False
    ...
    def data_ready(self):
        ...
        self._write_command(_CMD_READ_DATA_READY)
        data = self._read_data(1, execution_time=_EXEC_TIME_READ_DATA_READY)
        # Last bit indicates data ready status
        return bool(data[0] & 0x0001)
    
    @property
    def serial_number(self):
        """Get the sensor serial number as ASCII string (up to 32 characters)"""
        if self._serial_number is None:
            self._write_command(_CMD_READ_SERIAL_NUMBER)
            # Serial number is string<32> (16 words max)
            data = self._read_data(16, execution_time=_EXEC_TIME_STANDARD)
    ...
    def product_name(self):
        """Get the product name (32 bytes)"""
        if self._product_name is None:
            self._write_command(_CMD_READ_PRODUCT_NAME)
    ...
    def device_status(self):
        ...
        """
        self._write_command(_CMD_READ_DEVICE_STATUS)
        # Status register is 32 bits (2 words)
        data = self._read_data(2, execution_time=_EXEC_TIME_STANDARD)
    ...
    def clear_device_status(self):
        ...
        """
        self._write_command(_CMD_CLEAR_DEVICE_STATUS, execution_time=_EXEC_TIME_STANDARD)
    ...
    def start_fan_cleaning(self):
        ...
        """
        if self._measurement_started:
            raise RuntimeError("Cannot start fan cleaning while measuring. Call stop_measurement() first.")
        self._write_command(_CMD_START_FAN_CLEANING, execution_time=_EXEC_TIME_STANDARD)
    ...
    def get_version(self):
        ...
        """
        self._write_command(0xD100)  # GET_VERSION command
        data = self._read_data(1, execution_time=_EXEC_TIME_STANDARD)
    ...
    def get_sht_heater_measurements(self):
        """Get measurements when SHT heater is active (firmware >= 4.0)
        ...
        """
        self._write_command(0x6790)  # GET_SHT_HEATER_MEASUREMENTS
        data = self._read_data(2, execution_time=_EXEC_TIME_STANDARD)
        ...
        return {
            'humidity': None if data[0] == _UNKNOWN_VALUE_INT16 else data[0] / humidity_scale,
            'temperature': None if data[1] == _UNKNOWN_VALUE_INT16 else data[1] / temp_scale
        }
    
    def check_sensor_errors(self):
    ...
    @property
    def error_status_description(self):
        """Get human-readable description of current errors and their effects
...
class SEN66(SEN6x):
    ...
    def read_measurement(self):
        """Read all measurement values from SEN66
        ...
        """
        if not self._measurement_started:
            raise RuntimeError("Sensor must be in measurement mode. Call start_measurement() first.")
            
        self._write_command(_CMD_SEN66_READ_MEASUREMENT)
        
        # SEN66 returns 9 values (9 words) - includes CO2
        data = self._read_data(9, execution_time=_EXEC_TIME_READ_MEASUREMENT)
        ...
        pm1_0 = None if data[0] == _UNKNOWN_VALUE_UINT16 else data[0] / pm_scale
        pm2_5 = None if data[1] == _UNKNOWN_VALUE_UINT16 else data[1] / pm_scale
        pm4_0 = None if data[2] == _UNKNOWN_VALUE_UINT16 else data[2] / pm_scale
        pm10 = None if data[3] == _UNKNOWN_VALUE_UINT16 else data[3] / pm_scale
        
        # Process RH&T values (int16, 0x7FFF = unknown)
        humidity = None if data[4] == _UNKNOWN_VALUE_INT16 else data[4] / humidity_scale
        temperature = None if data[5] == _UNKNOWN_VALUE_INT16 else data[5] / temp_scale
        
        # Process VOC/NOx indices (int16, 0x7FFF = unknown)
        voc_index = None if data[6] == _UNKNOWN_VALUE_INT16 else data[6] / voc_nox_scale
        nox_index = None if data[7] == _UNKNOWN_VALUE_INT16 else data[7] / voc_nox_scale
        
        # Process CO2 (uint16, 0xFFFF = unknown)
        co2 = None if data[8] == _UNKNOWN_VALUE_UINT16 else data[8]
    ...
    def read_raw_values(self):
        """Read raw sensor values from SEN66
        ...
        self._write_command(_CMD_SEN66_READ_RAW_VALUES)
        data = self._read_data(5, execution_time=_EXEC_TIME_READ_MEASUREMENT)
        
        temp_scale = 200.0
        humidity_scale = 100.0
        
        return {
            'raw_humidity': None if data[0] == _UNKNOWN_VALUE_INT16 else data[0] / humidity_scale,
            'raw_temperature': None if data[1] == _UNKNOWN_VALUE_INT16 else data[1] / temp_scale,
            'raw_voc': None if data[2] == _UNKNOWN_VALUE_UINT16 else data[2],
            'raw_nox': None if data[3] == _UNKNOWN_VALUE_UINT16 else data[3],
            'raw_co2': None if data[4] == _UNKNOWN_VALUE_UINT16 else data[4]
        }
    
    def read_number_concentration(self):
        """Read particle number concentration values
        ...
        self._write_command(_CMD_READ_NUMBER_CONCENTRATION)
        data = self._read_data(5, execution_time=_EXEC_TIME_READ_MEASUREMENT)
        
        nc_scale = 10.0
        
        return {
            'nc_pm0_5': None if data[0] == _UNKNOWN_VALUE_UINT16 else data[0] / nc_scale,
            'nc_pm1_0': None if data[1] == _UNKNOWN_VALUE_UINT16 else data[1] / nc_scale,
            'nc_pm2_5': None if data[2] == _UNKNOWN_VALUE_UINT16 else data[2] / nc_scale,
            'nc_pm4_0': None if data[3] == _UNKNOWN_VALUE_UINT16 else data[3] / nc_scale,
            'nc_pm10': None if data[4] == _UNKNOWN_VALUE_UINT16 else data[4] / nc_scale
        }
    
    def set_temperature_acceleration(self, k=10.0, p=10.0, t1=10.0, t2=10.0):
        """Set temperature acceleration parameters for RH/T engine
        ...
        self._write_command(_CMD_SET_TEMP_ACCELERATION, data=data, execution_time=_EXEC_TIME_STANDARD)
    
    def force_co2_recalibration(self, target_co2_ppm):
        ...
        """
        if self._measurement_started:
            raise RuntimeError("Cannot recalibrate CO2 while measuring. Call stop_measurement() first.")
            
        # Send target CO2 concentration
        self._write_command(_CMD_FORCE_CO2_RECALIBRATION, data=[target_co2_ppm], 
                          execution_time=_EXEC_TIME_CO2_RECALIBRATION)
    ...
    @property
    def co2_automatic_self_calibration(self):
        """Get CO2 sensor automatic self-calibration (ASC) status
        
        Returns:
            bool: True if ASC is enabled, False if disabled
        """
        if self._measurement_started:
            raise RuntimeError("Cannot read CO2 ASC while measuring. Call stop_measurement() first.")
            
        self._write_command(_CMD_GET_SET_CO2_AUTO_CALIB)
        data = self._read_data(1, execution_time=_EXEC_TIME_STANDARD)
    ...
    def co2_automatic_self_calibration(self, enabled):
        """Set CO2 sensor automatic self-calibration (ASC) status
        ...
        self._write_command(_CMD_GET_SET_CO2_AUTO_CALIB, data=[status_word], 
                          execution_time=_EXEC_TIME_STANDARD)
    
    @property
    def ambient_pressure(self):
        """Get ambient pressure used for CO2 compensation
        
        Returns:
            int: Current ambient pressure in hPa (hectopascals)
        """
        self._write_command(_CMD_GET_SET_AMBIENT_PRESSURE)
        data = self._read_data(1, execution_time=_EXEC_TIME_STANDARD)
        return data[0]
    
    @ambient_pressure.setter
    def ambient_pressure(self, pressure_hpa):
        ...
        self._write_command(_CMD_GET_SET_AMBIENT_PRESSURE, data=[pressure_hpa], 
                          execution_time=_EXEC_TIME_STANDARD)
    
    @property
    def sensor_altitude(self):
        """Get sensor altitude used for CO2 compensation
        
        Returns:
            int: Current sensor altitude in meters above sea level
        """
        if self._measurement_started:
            raise RuntimeError("Cannot read altitude while measuring. Call stop_measurement() first.")
            
        self._write_command(_CMD_GET_SET_SENSOR_ALTITUDE)
        data = self._read_data(1, execution_time=_EXEC_TIME_STANDARD)
        return data[0]
    
    @sensor_altitude.setter
    def sensor_altitude(self, altitude_m):
        """Set sensor altitude for CO2 compensation
        ...
        """
        if self._measurement_started:
            raise RuntimeError("Cannot set altitude while measuring. Call stop_measurement() first.")
            
        if not 0 <= altitude_m <= 3000:
            raise ValueError("Altitude must be 0-3000 meters")
            
        self._write_command(_CMD_GET_SET_SENSOR_ALTITUDE, data=[altitude_m], 
                          execution_time=_EXEC_TIME_STANDARD)

    @property
    def voc_algorithm_state(self):
        """Get VOC algorithm state for backup/restore
        ...
        """
        self._write_command(_CMD_GET_VOC_STATE)
        data = self._read_data(4, execution_time=_EXEC_TIME_STANDARD)  # 4 words = 8 bytes
    ...
    def voc_algorithm_state(self, state):
        ...
        """
        if self._measurement_started:
            raise RuntimeError("Cannot set VOC state while measuring. Call stop_measurement() first.")
        ...
        self._write_command(_CMD_SET_VOC_STATE, data=data, execution_time=_EXEC_TIME_STANDARD)
    
    def get_nox_algorithm_tuning(self):
        """Get NOx algorithm tuning parameters
        
        Returns:
            dict: Current NOx algorithm parameters
        """
        if self._measurement_started:
            raise RuntimeError("Cannot read NOx tuning while measuring. Call stop_measurement() first.")
            
        self._write_command(_CMD_GET_NOX_TUNING)
        data = self._read_data(6, execution_time=_EXEC_TIME_STANDARD)
    ...
    def set_nox_algorithm_tuning(self, index_offset=1, learning_time_offset_hours=12,
                                 gating_max_duration_minutes=720, gain_factor=230):
        ...
        """
        if self._measurement_started:
            raise RuntimeError("Cannot set NOx tuning while measuring. Call stop_measurement() first.")
        ...
        data = [index_offset, learning_time_offset_hours, learning_time_gain_hours,
                gating_max_duration_minutes, std_initial, gain_factor]
        self._write_command(_CMD_SET_NOX_TUNING, data=data, execution_time=_EXEC_TIME_STANDARD)
    ...
    def temperature(self):
        ...
        return self._measurement_data['temperature']
    ...
    def humidity(self):
        ...
        if self._measurement_data is None:
            self.read_measurement()
        return self._measurement_data['humidity']
    ...
    def pm2_5(self):
        ...
        if self._measurement_data is None:
            self.read_measurement()
        return self._measurement_data['pm2_5']
    ...
    def voc_index(self):
        ...
        if self._measurement_data is None:
            self.read_measurement()
        return self._measurement_data['voc_index']
    ...
    def nox_index(self):
        ...
        if self._measurement_data is None:
            self.read_measurement()
        return self._measurement_data['nox_index']
    ...
    def co2(self):
        ...
        if self._measurement_data is None:
            self.read_measurement()
        return self._measurement_data['co2']
